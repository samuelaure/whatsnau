...
export function sanitizeForTelegram(obj: any): any {
    ...
    for (const [key, value] of Object.entries(obj)) {
        ...
        // Redact WhatsApp IDs
        if (lowerKey.includes('whatsappid') || lowerKey === 'id' && typeof value === 'string' && value.startsWith('wamid')) {
            sanitized[key] = redactWhatsAppId(value as string);
            continue;
        }

        // Redact message content
        if (lowerKey === 'content' || lowerKey === 'message' || lowerKey === 'text' || lowerKey === 'body') {
