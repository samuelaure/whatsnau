...
export class SequenceService {
  ...
  private static async triggerFollowUp(lead: any, stage: any) {
    ...
    if (stage.name.includes('M2')) {
      message =
        'Hola, soy yo de nuevo. Solo quería asegurarme de que viste mi mensaje anterior sobre el sistema de captura de leads. ¿Te interesaría charlar brevemente?';
    } else if (stage.name.includes('M3')) {
      message =
        'Entiendo que quizás no es el mejor momento. Si te parece bien, puedo enviarte consejos semanales gratuitos sobre IA para tu negocio. ¿Te interesaría?';
    }
